...
function isCreditCardNumber(ccn) {
  ...
  for (let i = num.length - 1; i >= 0; i -= 1) {
    ...
    sum += (evenFromTheEnd) ? dbl : n;
    evenFromTheEnd = !evenFromTheEnd;
  }
  return sum % 10 === 0;
}
...
function isBracketsBalanced(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    ...
    const theLastOne = temp.pop();
    if (left.indexOf(theLastOne) !== right.indexOf(str[i])) {
      return false;
    }
  }
...
function getCommonDirectoryPath(pathes) {
  ...
  let same = true;
  let ind = 0;
  const p = pathes.sort((a, b) => a.length - b.length);
  const l = p[0].length;
  for (let i = 0; i < l; i += 1) {
    for (let j = 1; j < pathes.length; j += 1) {
      ind = i;
      while (j < pathes.length) {
        if (p[0][i] !== pathes[j][i]) {
          same = false;
          break;
        }
